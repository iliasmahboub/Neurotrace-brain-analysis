...
@dataclass(frozen=True)
class RegionHierarchyCountSummary:
    """Hierarchy-aware region count summary including ancestor rollups."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.child_region_count <= 0:
            raise ValueError("child_region_count must be positive")
        if self.cell_count < 0:
            raise ValueError("cell_count must be non-negative")
        if self.atlas_resolution_um <= 0:
            raise ValueError("atlas_resolution_um must be positive")
